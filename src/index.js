...
module.exports = function toReadable(number) {
    ...
    if (numberToDigits.length === 1) {
        ...
        numberToDigits.length === 2 &&
        numberToDigits[0] >= 2 &&
        numberToDigits[0] <= 9
    ) {
        finNumber =
            dozens[numberToDigits[0]] +
            " " +
            readableNumbers[numberToDigits[1]];
        ...
        if (numberToDigits[1] === 0 && numberToDigits[2] === 0) {
            ...
            finNumber =
                readableNumbers[numberToDigits[0]] +
                " hundred " +
                readableNumbers[numberToDigits[2]];
        } else if (
            numberToDigits[2] === 0 &&
            numberToDigits[1] >= 2 &&
            numberToDigits[1] <= 9
            ...
            finNumber =
                readableNumbers[numberToDigits[0]] +
                " hundred " +
                dozens[numberToDigits[1]];
        } else if (numberToDigits[1] === 1) {
            finNumber =
                readableNumbers[numberToDigits[0]] +
                " hundred " +
                readableNumbers[Number("1" + numberToDigits[2])];
